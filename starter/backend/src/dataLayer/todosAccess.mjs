...
export const getTodos = async (userId) => {
    logger.info('JDC : Fetch all todo items for the user');
...
export const createTodo = async (newTodo) => {
    logger.info(`JDC : Create a new todo item with ID: ${newTodo.todoId}`);
    const command = new PutCommand({
        TableName: todosTable,
        Item: newTodo
    });
    await docClient.send(command);
    return newTodo; // new todo
}

export const updateTodo = async (userId, todoId, updateData) => {
    logger.info(`JDC : Modify todo item with ID: ${todoId}`);
...
export const deleteTodo = async (userId, todoId) => {
    logger.info(`JDC : Delete a todo item with ID: ${todoId}`);
...
export const saveImgUrl = async (userId, todoId, bucketName) => {
    try {
        ...
        logger.info(`JDC : Set an image URL for todo item: ${imageUrl}`);
        await docClient.send(command);
    } catch (error) {
        logger.error('Error while updating image URL', { error });
    }
}
